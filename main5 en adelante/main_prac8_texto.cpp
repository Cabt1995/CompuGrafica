...
void InitGL ( GLvoid )     // Inicializamos parametros
{
	...
	/*Si no activamos la textura, no inicializara la textura*/
	...
	t_img.LoadTGA("t_img.tga");
	t_img.BuildGLTexture();
	t_img.ReleaseImage();
}
...
void prisma (GLuint textura1, GLuint textura2)  //Funcion creacion prisma
{
	...
	/*Inicializamos la textura a colocar*/
		glBindTexture(GL_TEXTURE_2D, textura2);   // choose the texture to use.
		glBegin(GL_POLYGON);	//Front
			glColor3f(1.0,1.0,1.0);
			glNormal3f( 0.0f, 0.0f, 1.0f);
			/*Establecemos coordenadas para hacer mapeo de coordenadas con nuestra textura*/
...
void display ( void )   // Creamos la funcion donde se dibuja
{
	...
	glTranslatef(pos_camX, pos_camY, pos_camZ);
		...
		glPushMatrix();
		glTranslated(-1.91, 0, 0);
			glScalef(1.0, 1.0, 4.0);
			prisma(t_metal01.GLindex, t_metal01.GLindex,NULL);
		glPopMatrix();
		...
		glPushMatrix();
			glTranslated(0, 0, 0);
			glRotated(90, 0, 1, 0);
			glScalef(3.8, 1.0, 2.7);
			prisma(NULL, t_metal01.GLindex, t_Ajedrez2.GLindex);
		glPopMatrix();
	glPopMatrix();
